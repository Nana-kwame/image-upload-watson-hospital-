...
//Upload a new image with a description
...
//Get one image by it's hosID
server_1.app.get('/images/:hosID', (req, res, next) => {
    let hosID = req.params.hosID;
    let db;
    const details = { '_id': new ObjectID(hosID) };
    db.collection("imageupload").findOne(details, (err, image) => {
        if (err) {
            res.sendStatus(400);
        }
        //res.setHeader('Content-Type', 'image/jpeg');
